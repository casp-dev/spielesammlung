...
impl Meeple {
            ...
            for check_option_meeple in row {
                if let Some(check_meeple) = check_option_meeple {
                    if check_meeple.color != self.color {
                        if !self.check_casteling_chess_contains(
                            check_meeples.clone(),
                            check_meeple,
                            chess_board,
                        ) {
    ...
    fn check_casteling_chess_contains(
        &self,
        casteling_check_list: Vec<(usize, usize)>,
        check_meeple: &Meeple,
        chess_board: &[[Option<Meeple>; 8]; 8],
    ) -> bool {
        !check_meeple
            .show_moves(chess_board, &((42, 42), (42, 42)))
            .iter()
            .any(|&item| casteling_check_list.contains(&item))
    }
